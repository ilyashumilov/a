...
class FloodPosts(FloodCommand):
    def __init__(self, parsing_task_type_name: ParsingTaskTypeName):
        super(FloodPosts, self).__init__(parsing_task_type_name)
        self.base_parser = PostsFloodV2
...
async def worker():
    ...
    fc = FloodPosts(ParsingTaskTypeName.posts)
    await fc.flood_data()
